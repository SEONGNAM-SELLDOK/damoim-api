...
public class UsedItemSearchRepository {
    ...
    public Page<ResponseUsedItem> search(SearchUsedItemRequest request, Pageable pageable) {
        QueryResults<ResponseUsedItem> results = queryFactory
            ...
            .where(
                ...
                afterFrom(request.getFrom(), request.getTo()),
                beforeTo(request.getFrom(), request.getTo())
    ...
    private BooleanExpression afterFrom(LocalDate from, LocalDate to) {
        if (Objects.nonNull(from) && Objects.isNull(to)) {
            return usedItem.createDate.after(from.atStartOfDay());
        }
        return null;
    }

    private BooleanExpression beforeTo(LocalDate from, LocalDate to) {
        if (Objects.nonNull(to) && Objects.isNull(from)) {
            return usedItem.createDate.before(to.atStartOfDay().plusDays(1L));
        }
        return null;
    }
